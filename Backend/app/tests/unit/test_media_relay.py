...
async def test_relay_to_image_service():
    ...
    with patch(
        "app.core.media_relay.get_service",
        ...
        mock_get_service.assert_called_once_with("image")
...
async def test_relay_to_pdf_service():
    ...
    with patch(
        ...
        mock_get_service.assert_called_once_with("pdf")
        mock_service.analyse.assert_awaited_once_with(media_id)
...
@pytest.mark.asyncio
async def test_relay_to_video_service():
    media_id = uuid4()
    ...
    with patch(
        ...
        mock_get_service.assert_called_once_with("video")
        mock_service.analyse.assert_awaited_once_with(media_id)
